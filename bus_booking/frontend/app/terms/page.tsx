          ...
          <div className="flex gap-1 overflow-x-auto py-3 scrollbar-hide">
            {sections.map((s) => (
              <a
                key={s.num}
                href={`#section-${s.num}`}
                className="shrink-0 rounded-full px-3 py-1 text-xs font-medium transition-colors bg-indigo-50 dark:bg-indigo-950/40 text-indigo-700 dark:text-indigo-300 hover:bg-indigo-100 dark:hover:bg-indigo-900/60"
              >
                {s.title}
              </a>
            ))}
        ...
        {sections.map((s) => {
          ...
          return (
            <div
              key={s.num}
              id={`section-${s.num}`}
              className="rounded-2xl border p-6 md:p-8 scroll-mt-16 bg-indigo-50/60 dark:bg-indigo-950/20 border-indigo-100 dark:border-indigo-900"
            >
              {/* Section header */}
              <div className="flex items-start gap-4 mb-6">
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-white dark:bg-slate-900 shadow-sm border border-indigo-100 dark:border-indigo-900">
              ...
              {/* CTA link */}
              {"cta" in s && s.cta && (
                <div className="mb-5 rounded-xl border border-indigo-100 dark:border-indigo-900 bg-white dark:bg-slate-900/60 px-4 py-3 flex items-center justify-between">
                  <p className="text-sm text-slate-600 dark:text-slate-400">Need full refund details?</p>
                  <Link href={s.cta.href} className="text-sm font-semibold text-indigo-600 dark:text-indigo-400 hover:underline">
                    {s.cta.label} &rarr;
                  </Link>
                </div>
              )}
              ...
              {/* Top-level body */}
              {"body" in s && s.body && (
                <p className="text-sm leading-relaxed text-slate-700 dark:text-slate-300">{s.body as string}</p>
              )}
              ...
              {"points" in s && s.points && (
                <div className="space-y-5">
                  {(s.points as Array<{ heading: string; body?: string | null; list?: string[]; footer?: string }>).map((pt, i) => (
                    <div key={i} className="bg-white/70 dark:bg-slate-900/50 rounded-xl px-4 py-4">
                      <p className="text-xs font-bold uppercase tracking-wide mb-2 text-indigo-600 dark:text-indigo-400">
                        {pt.heading}
                      </p>
                      {pt.body && (
                        ...
                        <ul className="space-y-1.5 mt-1">
                          {pt.list.map((li, j) => (
                            <li key={j} className="flex items-start gap-2 text-sm text-slate-600 dark:text-slate-400">
                              <span className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-indigo-300 dark:bg-indigo-600" />
                              {li}
                            </li>
                          ))}
                        </ul>
